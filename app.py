...
def card_html(label, value, sub="", accent=False, warn=False):
    ...
    return f"""
    <div style="background:#111520;border:1px solid rgba(255,255,255,0.06);border-radius:10px;
...
def section_header(title, sub=""):
    st.markdown(f"""
    <div style="padding:20px 24px 12px;position:relative;z-index:1;">
    ...
    <div style="position:sticky;top:0;z-index:100;background:rgba(8,10,13,0.95);
      ...
      <div style="display:flex;align-items:center;gap:16px;">
        <div style="font-family:'Syne',sans-serif;font-size:13px;font-weight:700;
                    letter-spacing:.12em;color:#E8EDF5;">
          MARKETING<span style="color:#00E5A0;">ANALYTICS</span>
        </div>
        <div style="font-family:'DM Mono',monospace;font-size:10px;padding:3px 8px;
                    border-radius:3px;border:1px solid #00E5A0;color:#00E5A0;
                    letter-spacing:.06em;">CAMPAIGN DATASET</div>
      </div>
      <div style="display:flex;align-items:center;gap:8px;font-family:'DM Mono',monospace;
                  font-size:11px;color:#8896A8;">
        <span style="width:6px;height:6px;border-radius:50%;background:#00E5A0;
                     box-shadow:0 0 6px #00E5A0;display:inline-block;"></span>
        {periodo}
      </div>
    </div>""", unsafe_allow_html=True)
...
def render_budget_bar(gasto, budget):
    ...
    <div style="background:#111520;border:1px solid rgba(255,255,255,0.06);
      ...
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:#8896A8;white-space:nowrap;">
        BUDGET CONSUMIDO
      </div>
      <div style="flex:1;height:4px;background:rgba(255,255,255,0.06);border-radius:2px;overflow:hidden;">
        <div style="height:100%;width:{pct:.1f}%;background:{color};border-radius:2px;transition:width .8s;"></div>
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:12px;font-weight:500;
                  color:{color};white-space:nowrap;">{pct:.1f}%</div>
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:#8896A8;white-space:nowrap;">
        {fmt_usd(gasto)} / {fmt_usd(budget)}
      </div>
    </div>""", unsafe_allow_html=True)
...
def chart_cpc_mensal(df):
    ...
    agg = df2.groupby("mes").apply(
        lambda x: x["media_cost_usd"].sum() / x["clicks"].sum()
    ).reset_index(name="cpc")
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=6, color=[
            ACCENT3 if i == pico_idx else (ACCENT if i == min_idx else "#4A5568")
            for i in agg.index
        ], line=dict(width=0)),
        fill="tozeroy",
    ...
    fig.add_annotation(
        x=agg.loc[pico_idx, "mes"], y=agg.loc[pico_idx, "cpc"],
        text=f"Pico ${agg.loc[pico_idx,'cpc']:.2f}", showarrow=True,
        arrowhead=0, arrowcolor=ACCENT3, font=dict(color=ACCENT3, size=10),
        ax=0, ay=-30,
    )
    fig.add_annotation(
        x=agg.loc[min_idx, "mes"], y=agg.loc[min_idx, "cpc"],
        text=f"Mín ${agg.loc[min_idx,'cpc']:.2f}", showarrow=True,
        arrowhead=0, arrowcolor=ACCENT, font=dict(color=ACCENT, size=10),
        ax=0, ay=30,
    )
    fig.update_layout(**PLOTLY_LAYOUT, title=dict(
        text="CPC mensal — evolução Mai–Dez 2022",
        font=dict(size=11, color="#8896A8"), x=0,
    ), height=240)
    return fig
...
def chart_canal_ctr_cpc(df):
    if "channel_name" not in df.columns:
        return None
    agg = df.groupby("channel_name").agg(
        ctr=("ctr", "mean"),
        cpc=("cpc", "mean"),
        custo=("media_cost_usd", "sum"),
        cliques=("clicks", "sum"),
    ).reset_index().dropna()
    ...
    fig.add_trace(go.Bar(
        x=agg["channel_name"], y=agg["ctr"],
        name="CTR %", marker_color=cores,
        marker_line=dict(color=cores, width=1),
        hovertemplate="<b>%{x}</b><br>CTR: %{y:.2f}%<extra></extra>",
    ), secondary_y=False)

    fig.add_trace(go.Scatter(
        x=agg["channel_name"], y=agg["cpc"],
        name="CPC $", mode="lines+markers",
        line=dict(color=ACCENT5, width=2),
        marker=dict(size=7, color=ACCENT5),
        hovertemplate="<b>%{x}</b><br>CPC: $%{y:.4f}<extra></extra>",
    ), secondary_y=True)
    ...
    fig.update_layout(**layout,
        ...
        height=240,
        yaxis=dict(title="CTR (%)", gridcolor="rgba(255,255,255,0.04)"),
        yaxis2=dict(title="CPC ($)", gridcolor="rgba(0,0,0,0)"),
    )
    return fig

def chart_pareto(df):
    if "campaign_item_id" not in df.columns:
        return None
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color=ACCENT , marker_line_color=ACCENT, marker_line_width=0.5,
        name="Cliques", hovertemplate="Top %{x:.0f}% campanhas<br>Cliques: %{y:,.0f}<extra></extra>",
    ))
    fig.add_trace(go.Scatter(
        x=agg["rank_pct"], y=agg["pct_acum"],
    ...
    fig.add_hline(y=80, line_dash="dot", line_color=ACCENT3,
                  annotation_text="80%", annotation_font_color=ACCENT3,
                  yref="y2")
    if not corte_80.empty:
        fig.add_vline(x=corte_80["rank_pct"].values[0], line_dash="dot",
                      line_color=ACCENT3, opacity=0.5)

    fig.update_layout(**PLOTLY_LAYOUT,
        title=dict(text="Pareto — concentração de cliques por campanha",
                   font=dict(size=11, color="#8896A8"), x=0),
        height=260,
        xaxis=dict(title="% de campanhas (ranking)", gridcolor="rgba(255,255,255,0.04)"),
        yaxis=dict(title="Cliques", gridcolor="rgba(255,255,255,0.04)"),
...
def chart_weekday(df):
    if "weekday_cat" not in df.columns:
        return None
    df2 = df.copy()
    df2["periodo"] = df2["weekday_cat"].str.strip().str.lower().apply(
        lambda x: "Weekend" if str(x).startswith(("sat", "sun", "week_end")) else "Weekday"
    )
    agg = df2.groupby("periodo").agg(
        gasto=("media_cost_usd", "sum"),
        cliques=("clicks", "mean"),
    ).reset_index()

    fig = make_subplots(rows=1, cols=2,
        subplot_titles=["Gasto total ($)", "Média de cliques"],
        horizontal_spacing=0.12)

    cores = [ACCENT, ACCENT2]
    for i, (col, label) in enumerate([("gasto", "Gasto $"), ("cliques", "Cliques méd.")]):
        fig.add_trace(go.Bar(
            x=agg["periodo"], y=agg[col],
            marker_color=cores,
            marker_line=dict(color=cores, width=1),
    ...
    fig.update_layout(**PLOTLY_LAYOUT,
        title=dict(text="Weekday vs Weekend — gasto 5.7× maior, resultado igual",
                   font=dict(size=11, color="#8896A8"), x=0),
        height=220,
    )
...
def chart_fadiga(df):
    if "no_of_days" not in df.columns:
        return None
    df2 = df.copy()
    df2["no_of_days"] = pd.to_numeric(df2["no_of_days"], errors="coerce")
    df2 = df2.dropna(subset=["no_of_days"])
    df2["faixa"] = pd.cut(df2["no_of_days"],
        bins=[0, 7, 15, 9999],
        labels=["Curto\n0–7d", "Médio\n8–15d", "Longo\n+15d"])
    agg = df2.groupby("faixa", observed=True).agg(
        ctr=("ctr", "mean"), cpc=("cpc", "mean")
    ).reset_index().dropna()

    cores = [ACCENT2, ACCENT, ACCENT3]
    fig = make_subplots(rows=1, cols=2,
        subplot_titles=["CTR médio (%)", "CPC médio ($)"],
        horizontal_spacing=0.12)

    for col, row_i, fmt in [("ctr", 1, ".2f"), ("cpc", 2, ".3f")]:
        fig.add_trace(go.Bar(
            x=agg["faixa"].astype(str), y=agg[col],
            marker_color=cores,
            marker_line=dict(color=cores, width=1),
            showlegend=False,
            hovertemplate=f"<b>%{{x}}</b><br>%{{y:{fmt}}}<extra></extra>",
        ), row=1, col=row_i)

    fig.update_layout(**PLOTLY_LAYOUT,
        title=dict(text="Fadiga de criativo — ciclo de vida ideal: 15 dias",
                   font=dict(size=11, color="#8896A8"), x=0),
        height=220,
    )
    fig.update_annotations(font_size=10, font_color="#8896A8")
...
def chart_setembro_canal(df):
    ...
    fig = go.Figure(go.Bar(
        x=agg["cpc_set"], y=agg["channel_name"],
        orientation="h",
        marker_color=cores,
        marker_line=dict(color=cores, width=1),
    ...
    fig.update_layout(**PLOTLY_LAYOUT,
        title=dict(text="CPC Setembro por canal — janela de oportunidade Mobile",
                   font=dict(size=11, color="#8896A8"), x=0),
...
def chart_funil(total_imp, total_clicks, total_conv, total_receita):
    ...
    for label, val, color in stages:
        ...
        <div style="display:flex;align-items:center;gap:10px;">
          <div style="font-family:'DM Mono',monospace;font-size:10px;color:#8896A8;
                      width:70px;text-align:right;flex-shrink:0;">{label}</div>
          <div style="flex:1;height:28px;background:rgba(255,255,255,0.03);
                      border-radius:4px;overflow:hidden;position:relative;">
            <div style="height:100%;width:{max(pct,0.5):.1f}%;
                        background:{color}22;border-left:2px solid {color};
                        display:flex;align-items:center;padding-left:8px;transition:width .8s;">
              <span style="font-family:'DM Mono',monospace;font-size:10px;
                           font-weight:500;color:{color};white-space:nowrap;">{fmt}</span>
            </div>
          </div>
          <div style="font-family:'DM Mono',monospace;font-size:10px;color:#8896A8;
                      width:48px;text-align:right;flex-shrink:0;">{pct:.1f}%</div>
        </div>"""
    html += "</div>"
    return html
...
if uploaded is None:
    ...
    <div style="display:flex;align-items:center;justify-content:center;
      ...
      <div style="font-family:'DM Mono',monospace;font-size:40px;font-weight:300;
                  color:rgba(255,255,255,0.06);">◈</div>
      <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:600;color:#E8EDF5;">
        Marketing Analytics
      </div>
      <div style="font-family:'DM Mono',monospace;font-size:11px;color:#4A5568;text-align:center;
                  max-width:400px;line-height:1.8;">
        Faça upload do CSV exportado do PostgreSQL<br>
        no painel lateral para iniciar a análise.<br><br>
        <span style="color:#00E5A0;">72.613 campanhas · 5 canais · Mai–Dez 2022</span>
      </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
with st.container():
    ...
    with c1:
        st.markdown(card_html(
            "Investimento Total", fmt_usd(total_gasto),
            f"{total_gasto/budget*100:.1f}% do budget", accent=True
        ), unsafe_allow_html=True)
    with c2:
        st.markdown(card_html(
            "ROAS Global", f"{roas_global:.2f}x",
            f"{'↑' if roas_ok else '↓'} meta {meta:.1f}x",
            accent=roas_ok, warn=not roas_ok
        ), unsafe_allow_html=True)
    with c3:
        st.markdown(card_html(
            "Receita Estimada", fmt_usd(total_receita),
            f"Ticket R${ticket:.0f} · CVR {cvr:.1f}%"
        ), unsafe_allow_html=True)
    with c4:
        st.markdown(card_html(
            "CPA Médio", fmt_usd(cpa_medio),
            "custo por conversão"
        ), unsafe_allow_html=True)
    with c5:
        corr_txt = f"Pearson {corr_val:.4f}" if corr_val is not None else "—"
        st.markdown(card_html(
            "Correlação Budget×Clicks", f"{corr_val:.2f}" if corr_val is not None else "—",
            corr_txt, warn=(corr_val is not None and abs(corr_val) < 0.3)
        ), unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# Big cards row — Conversões + Funil
st.markdown('<div style="padding:0 24px 16px;">', unsafe_allow_html=True)
...
with col_big1:
    st.markdown(f"""
    <div style="background:#111520;border:1px solid rgba(255,255,255,0.06);
                border-radius:10px;padding:18px;height:180px;">
      <div style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;
                  color:#8896A8;margin-bottom:14px;">CONVERSÕES ESTIMADAS</div>
      <div style="font-family:'DM Mono',monospace;font-size:52px;font-weight:300;
                  color:#E8EDF5;line-height:1;">{fmt_k(total_conv)}</div>
      <div style="font-family:'DM Mono',monospace;font-size:10px;color:#00E5A0;
                  letter-spacing:.15em;text-transform:uppercase;margin-top:4px;">CONVERSÕES · CVR {cvr:.1f}%</div>
    </div>""", unsafe_allow_html=True)
...
with col_big2:
    ...
    <div style="background:#111520;border:1px solid rgba(255,255,255,0.06);
                border-radius:10px;padding:18px;height:180px;">
      <div style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;
                  color:#8896A8;margin-bottom:14px;">ALCANCE</div>
      <div style="font-family:'DM Mono',monospace;font-size:42px;font-weight:300;
                  color:#E8EDF5;line-height:1;">{fmt_k(total_imp)}</div>
      <div style="font-family:'DM Mono',monospace;font-size:10px;color:#00E5A0;
                  letter-spacing:.12em;text-transform:uppercase;margin-bottom:14px;">IMPRESSÕES TOTAL</div>
      <div style="display:flex;align-items:baseline;gap:8px;padding-top:10px;
...
with col_big3:
    ...
    <div style="background:#111520;border:1px solid rgba(255,255,255,0.06);
                border-radius:10px;padding:18px;height:180px;">
      <div style="font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;
                  color:#8896A8;margin-bottom:12px;">FUNIL DE CONVERSÃO</div>
      {funil_html}
    </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# SEGUNDA DOBRA — ANÁLISES OPERACIONAIS
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div style="padding:0 24px;">', unsafe_allow_html=True)
st.markdown('<hr style="border-color:rgba(255,255,255,0.06);margin:8px 0 0;">', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
# Row 1: CPC mensal + CTR/CPC por canal
...
# Insights box
st.markdown(f"""
<div style="background:#111520;border:1px solid rgba(0,229,160,0.15);border-radius:10px;
            padding:18px;margin-top:4px;">
  <div style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.12em;
              text-transform:uppercase;color:#00E5A0;margin-bottom:12px;">◈ CONCLUSÕES ESTRATÉGICAS</div>
...
with fc1:
    st.download_button(
        ...
        "marketing_campaign_raw.csv", "text/csv",
    )
with fc2:
    # agrega por canal para download
    ...
    if not agg_canal.empty:
        st.download_button(
            "⬇ Resumo por canal",
            agg_canal.to_csv(index=False).encode("utf-8"),
            "resumo_canal.csv", "text/csv",
        )
with fc3:
    if "campaign_item_id" in df.columns:
        ...
        st.download_button(
            "⬇ Top campanhas",
            agg_camp.to_csv(index=False).encode("utf-8"),
            "top_campanhas.csv", "text/csv",
        )

st.markdown(f"""
<div style="font-family:'DM Mono',monospace;font-size:10px;color:#4A5568;
            padding:16px 0 8px;line-height:2;">
